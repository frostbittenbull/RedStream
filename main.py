...
# ── Получить папку Downloads из реестра ───────────────────────────────────────
...
# ── Настройки (settings.txt рядом с exe) ──────────────────────────────────────
def get_settings_path():
    base = os.path.dirname(sys.executable) if getattr(sys, 'frozen', False) else os.path.dirname(os.path.abspath(__file__))
    return os.path.join(base, "settings.txt")

def load_dest_folder():
    """Читает папку из settings.txt, если файл есть. Иначе создаёт файл с папкой из реестра."""
...
def get_proxy_url():
    """Возвращает строку прокси для yt-dlp или None."""
    p = load_proxy()
...
# ── Лог-файл yt-dlp (log.txt рядом с exe) ─────────────────────────────────────
def get_log_path():
    base = os.path.dirname(sys.executable) if getattr(sys, 'frozen', False) else os.path.dirname(os.path.abspath(__file__))
    return os.path.join(base, "log.txt")


# ── История ссылок (history.txt рядом с exe) ──────────────────────────────────
...
def save_history(url):
    ...
    # Файл хранит всю историю без ограничений
    try:
...
def load_history_combo():
    """Возвращает только последние 5 ссылок для комбобокса."""
    return load_history()[:5]


# ── Показать окно в панели задач через WinAPI ─────────────────────────────────
...
def set_window_icon(hwnd):
    """Устанавливает иконку окна в панели задач через WinAPI."""
    try:
...
def make_combo(master, values, default, command=None):
    ...
    # Принудительное управление цветом стрелки
    def _on_enter(e):
...
def show_about(parent):
    ...
    meta_rows = [
        ...
        ("Версия:",    "2.3",                            "#aaaaaa", None),
        #("Сборка:",    _dt.date.today().strftime("%d.%m.%Y"), "#aaaaaa", None),
        ("Сборка:",    "28.08.2026",         "#aaaaaa", None),
...
def _bind_ctrl_keys(root, entry):
    """Ctrl+A/C/V/X/Z по коду клавиши — работает на любой раскладке клавиатуры."""
    def _ctrl_key(e):
        kc = e.keycode
        if kc == 65:    # A
            ...
            try: entry.event_generate("<<Undo>>")
            except Exception: pass
            return "break"
    entry.bind("<Control-KeyPress>", _ctrl_key, add="+")
...
def show_scheduler(toolbar_btn, app_root):
    ...
    if existing:
        try:
            if existing.winfo_exists():
                # НЕ создаем окно заново, а просто возвращаем скрытое окно на экран!
                pw = app_root._root_frame.winfo_width()
                ph = app_root._root_frame.winfo_height()
    ...
    _cal_popup = [None] # Поднимаем повыше для функции _close
    ...
    def _close():
        try:
            ...
            pass
        try:
            # Секрет здесь: мы больше НЕ используем popup.destroy(), 
            # чтобы таймер и введённые ссылки продолжали жить в фоне.
            popup.place_forget()
        except Exception:
            pass
    ...
    BOX_H = 24  # высота поля ввода (с pady=2 итого 28px)
    ...
    _bind_ctrl_keys(popup, date_entry._entry)
    ...
    def _add_url_row(text=""):
        ...
        _bind_ctrl_keys(popup, e._entry)
    ...
    def _sch_combo(values, default, col, on_change=None):
        ...
        # Добавляем привязку для клика по всему комбобоксу
        app_root._bind_combo_anywhere(cb)

        # Перехватываем _open_dropdown_menu и после открытия поднимаем все Toplevel
    ...
    def _update_status(text, color="#aaaaaa"):
        """Обновляет статус и сохраняет в глобальное состояние."""
    ...
    # Восстанавливаем статус если задача уже запущена
    ...
    # Позиционируем по центру приложения
    popup.update_idletasks()
    pw = app_root._root_frame.winfo_width()
    ph = app_root._root_frame.winfo_height()
    ww = popup.winfo_reqwidth()
    ...
    def _on_app_click(e):
        try:
            ...
            # Если клик был по самому окну планировщика
            ...
            # Важно: если клик был по кнопке "Планировщик" на тулбаре, игнорируем его
            # (иначе окно мгновенно закроется при попытке его открыть)
            ...
            # Проверяем, не открыто ли сейчас выпадающее окно (Toplevel) комбобокса
            import tkinter as _tk
            ...
            # Проверяем, не открыт ли виджет календаря
    ...
    # Отвязываем при закрытии попапа
    orig_close = _close
    def _close_and_unbind():
        try: app_root.unbind("<Button-1>", _bind_id)
        except Exception: pass
        orig_close()
    # Перепривязываем кнопку ✕ к новому _close
    try:
...
def show_extra(toolbar_btn, app_root):
    # Если дропдаун уже открыт — закрываем
...
def show_settings(toolbar_btn, app_root):
    ...
    # Позиционируем под кнопкой Настройки
    dropdown.update_idletasks()
    toolbar_btn.update_idletasks()
    bx = toolbar_btn.winfo_rootx() - app_root.winfo_rootx()
    by = toolbar_btn.winfo_rooty() - app_root.winfo_rooty() + toolbar_btn.winfo_height()
    ...
    def _update_ffmpeg():
        ...
        def _worker():
            try:
                ...
                popup.after(0, lambda: _finish("Ошибка", "error"))
...
class RedStreamApp(ctk.CTk):
    def __init__(self):
        ...
        # Панель с кнопками под стандартным заголовком
        ...
        # Сырые данные о форматах yt-dlp
        self._raw_formats = []
        
        # Словари для маппинга кодеков (внутреннее имя <-> отображаемое имя)
        ...
        # Папка назначения — берём из реестра
        ...
        def _late_binds():
            ...
            def _on_url_change(*_):
                # 1. Получаем текущий текст
                url = self.url_entry.get().strip()
                
                # 2. Если поле пустое или совпадает с предыдущим, ничего не делаем
                if not url or url == _prev_url[0]: 
                    return
                
                # 3. Автоподстановка протокола
                if not url.startswith(("http://", "https://")):
                    if "." in url:
                        url = "https://" + url
                        # Для ComboBox в CustomTkinter используем .set()
                        self.url_entry.set(url)

                # 4. Обновляем проверочную переменную
                _prev_url[0] = url

                # 5. Если теперь ссылка корректная, запускаем превью
    ...
    # ── Служебный метод для безопасной установки состояний и курсора комбобоксов ──
    ...
    def _check_ytdlp_update(self):
        def _worker():
            try:
                import urllib.request, json
                # Получаем текущую версию yt-dlp.exe
                ytdlp = resource_path("yt-dlp.exe")
    ...
    def _check_ffmpeg_update(self):
        def _worker():
            try:
                ...
                # Ищем дату (например, 2026-03-12) в строке "ffmpeg version 2026-03-12-git..."
                m_local = _re.search(r"(\d{4}-\d{2}-\d{2})", output)
                if not m_local: return
                current = m_local.group(1)
                
                # 2. Получаем версию с сервера gyan.dev
                req = urllib.request.Request(
                    "https://www.gyan.dev/ffmpeg/builds/ffmpeg-git-essentials.7z.ver",
                    headers={"User-Agent": "RedStream"}
                )
                with urllib.request.urlopen(req, timeout=10) as r:
                    # Сервер возвращает строку формата "2026-03-18-git-106616f13d"
                    online_ver = r.read().decode("utf-8").strip()
                
                # Достаем дату из ответа сервера
    ...
    def _next_banner_pos(self):
        """Возвращает (parent, y) для следующего баннера."""
    ...
    def _build_ui(self):
        ...
        # ── Горизонтальный блок: левая + правая колонки ────────────────────────
        columns = ctk.CTkFrame(p, fg_color="transparent")
        columns.pack(fill="x", padx=10, pady=(0, 6))
        columns.columnconfigure(0, weight=1)
        columns.columnconfigure(1, weight=0)  # разделитель
        ...
        # ── НИЖНЯЯ ЧАСТЬ: папка, кнопка, прогресс ─────────────────────────────
    ...
    def _show_download_settings(self):
        ...
        def _make_timecode_widget(parent):
            """Создаёт виджет ЧЧ:ММ:СС со статическими двоеточиями."""
    ...
    def _fetch_preview(self, url):
        def _worker():
            try:
                ...
                # Парсим форматы динамически и полностью исключаем дубликаты
                ...
                for f in self._raw_formats:
                    # Пропускаем сториборды и пустые треки
                ...
                # AV1 всегда присутствует в списке (если его нет — будет использован -S vcodec:av01)
                ...
                # Сортируем разрешения по высоте
                def res_key(r):
                    m = re.search(r'x(\d+)', r)
    ...
    def _calc_filesize(self, *_):
        ...
        # Декодируем косметические названия обратно в системные
        ...
        if not is_audio_only:
            ...
            for f in self._raw_formats:
                ...
                # Маппинг альтернативных названий кодеков
        ...
        if valid_a:
            # Выбираем самую качественную аудиодорожку
        ...
        if is_audio_only:
            ...
            # Если в видеопотоке уже есть звук (acodec != none), берем только его размер
            if best_v and best_v.get('acodec') != 'none':
                total_size = v_size
            else:
                total_size = v_size + a_size

        # Если выбран AV1 но его нет — пересчитываем для реального кодека и показываем предупреждение
        av1_warn = ""
        if not is_audio_only and sel_vc == "av01":
            ...
            if not av1_available and not valid_v:
                # Ищем лучший доступный видеопоток без фильтра по кодеку
        ...
        # Предупреждение для аудио форматов (OPUS/MP3) если кодек недоступен
        audio_warn = ""
    ...
    def _run_download(self):
        ...
        # ── Строим аргументы форматов из динамических комбобоксов
        sel_ext = self._format_map.get(self.format_combo.get(), "mp4")
        ...
        # Декодируем косметические названия обратно
        sel_vc = self.VC_RAW.get(sel_vc_disp, sel_vc_disp.lower()) if sel_vc_disp != "-" else "-"
        sel_ac = self.AC_RAW.get(sel_ac_disp, sel_ac_disp.lower()) if sel_ac_disp != "-" else "-"
        ...
        fmt_opts = []
        if is_audio:
            # Проверяем доступность нужного аудиокодека
            A_PREF = {"opus": ["opus"], "mp4a": ["mp4a", "aac"]}
            wanted_ac = sel_ac if sel_ac and sel_ac != "-" else "opus"
            prefixes = A_PREF.get(wanted_ac, [wanted_ac])
            ac_available = any(
            ...
            if not ac_available:
                # Нужный кодек недоступен — берём лучшее что есть
                fmt_opts = ["-f", "bestaudio/best", "-x", "--audio-format", sel_ext]
                # Находим реальный кодек для предупреждения
                real_ac = ""
                for f in getattr(self, "_raw_formats", []):
                    ac = f.get("acodec", "none")
                    if ac != "none":
            ...
            # Если выбран AV1 — проверяем, есть ли он реально в форматах
            av1_available = any(
                f.get("vcodec", "").startswith("av01")
                for f in getattr(self, "_raw_formats", [])
            )
            ...
            if sel_vc == "av01" and not av1_available:
                # AV1 недоступен напрямую — просим yt-dlp выбрать максимально близкий
                v_str = "[" + "][".join(v_cond) + "]" if v_cond else ""
        ...
        def run():
            ...
            # Дописываем лог в постоянный файл
            try:
